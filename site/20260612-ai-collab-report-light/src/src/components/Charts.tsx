...
export function HorizontalBarChart({ rows, maxVal }: HBarProps) {
  ...
  return (
    <svg viewBox={`0 0 ${totalW} ${totalH}`} style={{ width: "100%", height: "auto" }}>
      {/* header */}
      <text x={labelW} y={16} fontSize={11} fill="#6B7280" fontFamily="monospace">傳統工時</text>
      <text x={labelW + barAreaW * 0.45} y={16} fontSize={11} fill="#6B7280" fontFamily="monospace">AI 協作</text>
      ...
      {rows.map((row, i) => {
        ...
        return (
          <g key={i}>
            {/* label */}
            <text x={labelW - 12} y={y + 16} fontSize={12} fill="#374151"
...
export function DonutChart({ pct, label, sublabel, color, bg }: DonutProps) {
  ...
  return (
    <svg viewBox="0 0 220 220" style={{ width: 220, height: 220, flexShrink: 0 }}>
      ...
      <text x={cx} y={cy + 16} textAnchor="middle" fontSize={13} fill="#1C1C1E"
        fontFamily="Noto Sans SC, sans-serif">{label}</text>
      <text x={cx} y={cy + 34} textAnchor="middle" fontSize={11} fill="#6B7280"
    ...
    <svg viewBox={`0 0 ${w} ${h}`} style={{ width: "100%", height: "auto" }}>
      ...
      {series.map((s, i) => (
        <g key={s.label} transform={`translate(${padL + i * 148}, 8)`}>
          ...
          <text x={16} y={12} fontSize={13} fill="#374151"
      ...
      {yTicks.map((t) => (
        <g key={t}>
          ...
          <text x={padL - 8} y={py(t) + 4} fontSize={11} fill="#6B7280"
      ...
      {/* x labels */}
      {xLabels.map((l, i) => (
        <text key={i} x={px(i)} y={h - 4} fontSize={12} fill="#6B7280"
